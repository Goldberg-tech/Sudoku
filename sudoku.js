...
document.addEventListener('touchmove', e=>{
  ...
  const atTop=sa.scrollTop<=0, atBot=sa.scrollTop+sa.clientHeight>=sa.scrollHeight-1;
  const goDown=e.touches[0].clientY>touchStartY, goUp=e.touches[0].clientY<touchStartY;
...
function saveGameState(){
  ...
  ss('active_game',{puzzle,solution,userGrid,notesRaw,timerSec,currentLevel,isDaily,errorsCount,hintsLeft,notesMode,selected:selected?{row:selected.row,col:selected.col}:null});
}
...
function resumeGameFromSave(state){
  ...
  hintsLeft=state.hintsLeft??FREE_HINTS;notesMode=state.notesMode||false;
...
function undoMove(){
  ...
  if(history.length===0){
    document.getElementById('undo-overlay').classList.add('open');return;
  }
...
function startRewardedVideo(type){
  const totalSec=5;
  let elapsed=0;
  const barEl=document.getElementById(`${type}-rv-bar`);
  const skipEl=document.getElementById(`${type}-rv-skip`);
  const labelEl=document.getElementById(`${type}-rv-label`);
  const actionBtn=document.getElementById(type==='hint'?'btn-watch-hint-ad':'btn-watch-undo-ad');

  barEl.style.width='0%';
  skipEl.textContent=totalSec;
  skipEl.classList.remove('ready');
  skipEl.disabled=true;
  actionBtn.disabled=true;
  labelEl.textContent='Смотрите рекламу...';

  // Когда будет реальная РСЯ/ВК — заменить этот таймер на callback от SDK
  ...
  rvTimers[type]=setInterval(()=>{
    ...
    if(remaining>0){
      ...
      labelEl.textContent='Реклама завершена — получайте награду!';
    }
  },1000);
}
...
function stopRewardedVideo(type){
  clearInterval(rvTimers[type]);
  const skipEl=document.getElementById(`${type}-rv-skip`);
  if(skipEl)skipEl.classList.remove('ready');
}
let toastTimer;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Hint rewarded video
  document.getElementById('hint-overlay').addEventListener('transitionend',e=>{
    if(e.target.id==='hint-overlay'&&e.target.classList.contains('open'))startRewardedVideo('hint');
  });
  document.getElementById('hint-rv-skip').addEventListener('click',()=>{
    if(document.getElementById('hint-rv-skip').classList.contains('ready')){
      document.getElementById('btn-watch-hint-ad').disabled=false;
    }
  });
  document.getElementById('btn-watch-hint-ad').addEventListener('click',()=>{
    ...
    // Реальный callback rewarded видео РСЯ/ВК будет здесь
    hintsLeft=1;
  ...
  // Undo rewarded video
  document.getElementById('undo-overlay').addEventListener('transitionend',e=>{
    if(e.target.id==='undo-overlay'&&e.target.classList.contains('open'))startRewardedVideo('undo');
  });
  document.getElementById('undo-rv-skip').addEventListener('click',()=>{
    if(document.getElementById('undo-rv-skip').classList.contains('ready')){
      document.getElementById('btn-watch-undo-ad').disabled=false;
    }
  });
