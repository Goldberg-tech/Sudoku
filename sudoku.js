...
function initUserId() {
  ...
  // fallback — стабильный анонимный id в localStorage
...
function saveGameState() {
  ...
  ss('active_game', {
    puzzle, solution, userGrid, notesRaw,
    timerSec, currentLevel, isDaily,
    errorsCount, hintsLeft, notesMode,
...
function resumeGameFromSave(state) {
  ...
  hintsLeft = state.hintsLeft ?? FREE_HINTS;
  notesMode = state.notesMode || false;
  history = [];
  gameWon = false; gameLost = false; gamePaused = false; selected = null;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // 1. Инициализация MaxBridge + userId
  const tg = window.MaxBridge;
  if (tg) {
    try{tg.ready();}catch{}
    try{tg.expand();}catch{}
  ...
  initUserId(); // теперь userId известен — ключи хранилища корректны

  // сохраняем имя пользователя
  try {
    const user = tg?.initDataUnsafe?.user;
    const name = user?.name || user?.first_name || user?.username;
    if (name) ss('username', name);
  } catch {}
  ...
  // 3. Кнопка «Назад» — сохраняем игру перед выходом
  ...
  // 11. Сохраняем игру при уходе из вкладки / закрытии
  document.addEventListener('visibilitychange',()=>{
    if (document.hidden) saveGameState();
  });
  window.addEventListener('pagehide', saveGameState);

  // 12. Рендер главной — и если есть незавершённая партия, показываем баннер
  renderHome();

  // Проверяем наличие сохранённой игры
  const saved = loadGameState();
  if (saved && saved.puzzle && saved.puzzle.length) {
    // Добавляем баннер «Продолжить игру» на главную
...
function showResumeButton(saved) {
  ...
  banner.innerHTML = `
    <div>
      <div style="font-size:11px;font-weight:900;color:var(--accent);letter-spacing:1px;margin-bottom:4px;">НЕЗАВЕРШЁННАЯ ИГРА</div>
    ...
    <button id="btn-resume-game" style="
      padding:9px 16px;border-radius:9px;background:var(--accent);color:#1a1d2e;
      font-size:13px;font-weight:800;border:none;cursor:pointer;white-space:nowrap;
    ">▶ Продолжить</button>
  `;
  const pad = document.querySelector('#screen-home .home-pad');
  const dailyCard = pad.querySelector('.daily-card');
  pad.insertBefore(banner, dailyCard);
