...
function track(event,params){
  try{
    if(typeof ym!=='undefined'){ym(108242784,'reachGoal',event,params);}
  }catch(e){}
...
function renderHome(){
  ...
  if(saved&&saved.puzzle&&saved.puzzle.length){showResumeButton(saved);}
...
// ИСПРАВЛЕНО: убран дублирующий вызов reportGameResult, добавлен const hist
function saveResult(){
  const stats=getStats();
...
function shareResult(){
  const lvl=LEVELS.find(l=>l.id===currentLevel);
  const text=`Решил судоку! ${lvl?.name||''}\n⏱ ${formatTime(timerSec)}${errorsCount?`\nОшибок: ${errorsCount}`:''}\n\nИграй: https://max.ru/${window.MaxBridge?.initDataUnsafe?.user?.username||''}?startapp`;
...
function renderProfile(){
  ...
  if(avatarB64){avatarEl.innerHTML=`<img src="${avatarB64}" alt="avatar"/>`;}
  else{avatarEl.innerHTML=name[0]?.toUpperCase()||'?';}
...
function getMyLbEntry(){
  ...
  return{name:getUserName(),avatar:getUserAvatar(),solved:total,best:bestOverall,streak:stats.streak||0,
    levels:{easy:stats.easy?.solved||0,medium:stats.medium?.solved||0,hard:stats.hard?.solved||0,expert:stats.expert?.solved||0,master:stats.master?.solved||0},isMe:true};
}
function getLbSortKey(entry,level){if(level==='all')return entry.solved;return entry.levels[level]||0;}
...
function renderLeaderboard(){
  ...
  const myPos=document.createElement('div');myPos.className='lb-my-pos';
  ...
  myPos.innerHTML=`
    ...
    <div class="lb-info"><div class="lb-name">Вы — ${me.name}</div><div class="lb-sub" style="color:rgba(255,255,255,.5);">Серия: ${me.streak} дн.</div></div>
    <div style="text-align:right;"><div class="lb-stat">${myScore}</div><div style="font-size:10px;color:rgba(255,255,255,.5);">#${myRank} место</div></div>
  `;
  list.appendChild(myPos);
  const topTitle=document.createElement('div');topTitle.className='lb-section-title';
  topTitle.textContent=`Топ игроков · ${_lbLevel==='all'?'Все уровни':LEVELS.find(l=>l.id===_lbLevel)?.name||''}`;
  list.appendChild(topTitle);
  allEntries.forEach((entry,i)=>{
    const rank=i+1;
    const row=document.createElement('div');row.className='lb-row'+(entry.isMe?' me':'');
    ...
    row.innerHTML=`
      ...
      <div class="lb-info"><div class="lb-name">${entry.name}${entry.isMe?' (вы)':''}</div><div class="lb-sub">Серия: ${entry.streak} дн.</div></div>
      <div class="lb-stat">${score}</div>
    `;
    list.appendChild(row);
  });
  const note=document.createElement('div');note.style.cssText='font-size:10px;color:var(--muted);text-align:center;padding:12px 0;';
...
function startRewardedVideo(type,onComplete){
  const totalSec=5;let elapsed=0;
  ...
  barEl.style.width='0%';claimBtn.disabled=true;
  ...
  rvTimers[type]=setInterval(()=>{
    ...
    if(elapsed>=totalSec){clearInterval(rvTimers[type]);claimBtn.disabled=false;labelEl.textContent='Готово! Забирайте награду.';}
  },1000);
}
...
function handleAvatarUpload(file){
  ...
  reader.onload=e=>{saveUserAvatar(e.target.result);renderProfile();showToast('Фото обновлено');};
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.querySelectorAll('.lb-tab').forEach(tab=>{
    tab.addEventListener('click',()=>{
      ...
      tab.classList.add('active');_lbLevel=tab.dataset.level;renderLeaderboard();
    });
  });
  document.getElementById('btn-back').addEventListener('click',()=>{saveGameState();stopTimer();showScreen('home');});
  ...
  document.getElementById('btn-watch-hint-ad').addEventListener('click',()=>{
    ...
    startRewardedVideo('hint',()=>{hintsLeft=1;document.getElementById('hint-badge').textContent=1;applyHint();});
  });
  document.getElementById('btn-hint-cancel').addEventListener('click',()=>{document.getElementById('hint-overlay').classList.remove('open');});
  document.getElementById('hint-rv-claim').addEventListener('click',()=>{const cb=rvCallbacks['hint'];stopRewardedVideo('hint');if(cb)cb();});
  document.getElementById('hint-rv-cancel').addEventListener('click',()=>{stopRewardedVideo('hint');});
  document.getElementById('btn-watch-undo-ad').addEventListener('click',()=>{
    ...
    startRewardedVideo('undo',()=>{applyUndo();});
  });
  document.getElementById('btn-undo-cancel').addEventListener('click',()=>{document.getElementById('undo-overlay').classList.remove('open');});
  document.getElementById('undo-rv-claim').addEventListener('click',()=>{const cb=rvCallbacks['undo'];stopRewardedVideo('undo');if(cb)cb();});
  document.getElementById('undo-rv-cancel').addEventListener('click',()=>{stopRewardedVideo('undo');});
  document.getElementById('btn-close-stats').addEventListener('click',()=>{document.getElementById('stats-overlay').classList.remove('open');});
  ...
  document.getElementById('btn-cancel-name').addEventListener('click',()=>{document.getElementById('edit-name-overlay').classList.remove('open');});
  document.getElementById('edit-name-input').addEventListener('keydown',e=>{if(e.key==='Enter')saveNameAndClose();});
  document.getElementById('btn-avatar-edit').addEventListener('click',()=>{document.getElementById('avatar-file-input').click();});
  document.getElementById('avatar-file-input').addEventListener('change',e=>{handleAvatarUpload(e.target.files[0]);e.target.value='';});
