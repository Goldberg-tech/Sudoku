...
function updateUndoBadge(){
  const el=document.querySelector('#btn-undo .tbtn-lbl');
  if(!el)return;
  if(undosLeft>0){
    el.textContent='Отменить';
    document.querySelector('#btn-undo .tbtn-icon').style.opacity='1';
  }else{
    el.textContent='Отменить';
    document.querySelector('#btn-undo .tbtn-icon').style.opacity='1';
  }
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('hint-rv-skip').addEventListener('click',()=>{/* skip handled by claim button */});
  ...
  document.getElementById('undo-rv-skip').addEventListener('click',()=>{/* skip handled by claim button */});
